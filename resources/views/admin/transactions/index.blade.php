...
@section('content')
    <div class="card glass-card border-0 mb-4">
        <div
            class="card-header bg-transparent border-0 p-4 d-flex flex-wrap gap-3 justify-content-between align-items-center">
            ...
            <form class="d-flex flex-wrap gap-2" method="GET">
                <div class="input-group input-group-sm rounded-pill overflow-hidden shadow-sm" style="width:250px;">
                    ...
                    <input type="text" name="search" class="form-control border-0 ps-0"
                        placeholder="Ketik No. Transaksi/Pasien..." value="{{ request('search') }}">
                </div>

                <select name="status" class="form-select form-select-sm rounded-pill shadow-sm border-0 px-3"
                    style="width:130px">
                ...
                <div class="input-group input-group-sm rounded-pill overflow-hidden shadow-sm flex-nowrap"
                    style="width: auto;">
                    <span class="input-group-text bg-white border-0 text-muted"><i class="bi bi-calendar3"></i></span>
                    <input type="date" name="from" class="form-control border-0 px-2" value="{{ request('from') }}"
                        title="Dari Tanggal">
                    <span class="input-group-text bg-light border-0 text-muted px-2">-</span>
                    <input type="date" name="to" class="form-control border-0 px-2" value="{{ request('to') }}"
                ...
                @if(request()->anyFilled(['search', 'status', 'from', 'to']))
                    <a href="{{ url()->current() }}"
                        class="btn btn-sm btn-outline-danger rounded-pill px-3 shadow-sm btn-action">
                        ...
                        <tr>
                                ...
                                <div class="d-flex align-items-center">
                                    ...
                                    <div>
                                        <div class="fw-semibold text-dark">
                                            {!! $trx->patient->nama ?? '<span class="text-muted small">Umum</span>' !!}</div>
                            ...
                            <td>
                                <span class="text-muted small"><i
                                        class="bi bi-headset me-1"></i>{{ $trx->kasir->name ?? '-' }}</span>
                            </td>
                            <td class="fw-bold">Rp {{ number_format($trx->total_bayar, 0, ',', '.') }}</td>
                            <td>
                                @if($trx->potongan_bpjs > 0)
                                    <span class="text-success fw-semibold"><i class="bi bi-shield-check me-1"></i>Rp
                                        {{ number_format($trx->potongan_bpjs, 0, ',', '.') }}</span>
                                @else
                            ...
                            <td class="text-end pe-4">
                                <a href="{{ route('transactions.show', $trx) }}"
                                    class="btn btn-sm btn-primary rounded-circle shadow-sm btn-action"
